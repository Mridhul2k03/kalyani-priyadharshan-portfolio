
export const films = [
    {
        id: "lokah-chapter-1-chandra",
        title: "Lokah Chapter 1: Chandra",
        role: "Chandra",
        year: "2025",
        genre: "Fantasy Action",
        trailer: "https://www.youtube.com/embed/64XHtNWTB5o",
        status: "Released",
        description: "A warrior princess fights to save her realm from encroaching darkness in this high-octane fantasy epic.",
        fullDescription: "Lokah Chapter 1: Chandra is a grand fantasy action saga that transports viewers to a mystical realm Tejas. Kalyani Priyadarshan stars as Chandra, a fierce warrior princess destined to lead her people against an ancient evil. With breathtaking visuals, high-octane action sequences, and a gripping storyline, the film explores themes of destiny, sacrifice, and the enduring power of hope.",
        director: "Dominic Arun",
        cast: ["Kalyani Priyadarshan", "Naslen", "Dulquer Salmaan"],
        images: [
            "https://wallpapercave.com/wp/wp15785948.jpg",
        ]
    },
    {
        id: "genie",
        title: "Genie",
        role: "Priya",
        year: "2024",
        genre: "Sci-Fi Thriller",
        trailer: "https://www.youtube.com/embed/dfK4zEk5cVk",
        status: "Post-Production",
        description: "In a world where memories can be traded, a young woman discovers a conspiracy that threatens reality itself.",
        fullDescription: "Genie is a mind-bending sci-fi thriller featuring Jayam Ravi and Kalyani Priyadarshan. The film delves into a futuristic society where human memories can be extracted and traded as commodities. Kalyani plays Priya, a character caught in a web of corporate espionage and existential questioning.",
        director: "Bhuvanesh Arjunan",
        cast: ["Jayam Ravi", "Kalyani Priyadarshan", "Krithi Shetty"],
        images: [
            "https://m.media-amazon.com/images/M/MV5BN2IyYzY0NTYtMjljOS00OTZlLTliMTAtZGQ4YmJiN2ViOGI0XkEyXkFqcGc@._V1_FMjpg_UY2250_.jpg"
        ]
    },
    {
        id: "varshangalkku-shesham",
        title: "Varshangalkku Shesham",
        role: "Annie",
        year: "2024",
        genre: "Drama",
        trailer: "https://www.youtube.com/embed/50GJ7D_00NA",
        status: "Released",
        description: "A poignant tale of friendship and artistic struggle spanning decades in the golden era of cinema.",
        fullDescription: "A nostalgic journey through the golden years of Malayalam cinema, Varshangalkku Shesham follows two friends who travel to Madras with dreams of making it big in the film industry. Kalyani Priyadarshan delivers a touching performance as Annie, navigating the complexities of fame and relationships.",
        director: "Vineeth Sreenivasan",
        cast: ["Pranav Mohanlal", "Dhyan Sreenivasan", "Kalyani Priyadarshan"],
        images: ["https://m.media-amazon.com/images/M/MV5BNjA0NDlmZDctNDg3NS00OWU5LWEyNTQtZGQ1MGMyZjM1NDAyXkEyXkFqcGc@._V1_FMjpg_UY2048_.jpg"
        ]
    },
    {
        id: "sesham-mike-il-fathima",
        title: "Sesham Mike-il Fathima",
        role: "Fathima",
        year: "2023",
        genre: "Comedy Drama",
        trailer: "https://www.youtube.com/embed/V1XvyC_xRZ8",
        status: "Released",
        description: "A spirited young woman from a conservative background fights to become a football commentator.",
        fullDescription: "Fathima, a young woman from Malappuram, dreams of becoming a football commentator—a field dominated by men. Sesham Mike-il Fathima is a heartwarming and humorous tale of her breaking barriers, silencing critics, and finding her voice, quite literally.",
        director: "Manu C Kumar",
        cast: ["Kalyani Priyadarshan", "Femina George", "Shaheen Siddique"],
        images: [
            "https://m.media-amazon.com/images/M/MV5BMTZkNTJhMTgtZGI2NS00NTEyLWIwMTMtZWY5OTA2YWI4YmYwXkEyXkFqcGc@._V1_FMjpg_UX986_.jpg"
        ]
    },
    {
        id: "antony",
        title: "Antony",
        role: "Ann Maria",
        year: "2023",
        genre: "Action Drama",
        trailer: "https://www.youtube.com/embed/ZE8OIPadmGA",
        status: "Released",
        description: "A volatile gangster finds redemption through an unexpected bond with a young student.",
        fullDescription: "Antony is an emotionally charged action drama exploring the relationship between a ruthless gangster and a young girl who enters his life. Kalyani Priyadarshan plays Ann Maria, a college student who becomes the catalyst for Antony's transformation.",
        director: "Joshiy",
        cast: ["Joju George", "Kalyani Priyadarshan", "Nyla Usha"],
        images: [
            "https://img.airtel.tv/unsafe/fit-in/1600x0/filters:format(webp)/https://ultra-assets.streamready.in/assets/ULTRA/MOVIE/68885d6f006c066bd89bb348/images/LANDSCAPE_169/LANDSCAPE_169_kc6Jpb0Mm_Antony04_1920x1080-(1).jpg?o=production"
        ]
    },
    {
        id: "thallumaala",
        title: "Thallumaala",
        role: "Fathima Beevi",
        year: "2022",
        genre: "Action Comedy",
        trailer: "https://www.youtube.com/embed/s_OdRGbpKUA",
        status: "Released",
        description: "A non-linear stylistic action film following the chaotic life of an internet celebrity and his rivals.",
        fullDescription: "Thallumaala is a vibrant, chaotic, and stylish action entertainer that became a cult hit. It follows Wazim, an internet celebrity, and a series of brawls ('Thallu') that define his life. Kalyani plays Fathima Beevi, a vlogger and the love interest who adds to the colorful chaos.",
        director: "Khalid Rahman",
        cast: ["Tovino Thomas", "Kalyani Priyadarshan", "Shine Tom Chacko"],
        images: [
            "https://m.media-amazon.com/images/M/MV5BYjgyMDA2ZTMtYzAyMS00YjQ2LWI2NTEtMDYxOWM5OTE5NTRkXkEyXkFqcGc@._V1_FMjpg_UX986_.jpg"
        ]
    },
    {
        id: "bro-daddy",
        title: "Bro Daddy",
        role: "Anna",
        year: "2022",
        genre: "Family Comedy",
        trailer: "https://www.youtube.com/embed/zdCLPqEHHew",
        status: "Released",
        description: "A fun-filled family entertainer about a father and son who accidentally fall in love with the same woman's daughter... sort of.",
        fullDescription: "A hilarious family drama centered on a rich Christian family in Kerala. Bro Daddy explores the complications that arise when a father and son try to navigate their respective love lives. Kalyani plays Anna, the vibrant daughter of a family friend, who gets entangled in the confusion.",
        director: "Prithviraj Sukumaran",
        cast: ["Mohanlal", "Prithviraj Sukumaran", "Kalyani Priyadarshan", "Meena"],
        images: [
            "https://m.media-amazon.com/images/M/MV5BMTgwNTBiMjEtNmJiNS00NWZlLTk3ZDUtMjQ5M2MwMGI2MTAxXkEyXkFqcGc@._V1_FMjpg_UY2048_.jpg"
        ]
    },
    {
        id: "hridayam",
        title: "Hridayam",
        role: "Nithya",
        year: "2022",
        genre: "Coming-of-age",
        trailer: "https://www.youtube.com/embed/48tqyDA5Sb0",
        status: "Released",
        description: "A journey of a young man from his college days to adulthood, exploring his relationships and personal growth.",
        fullDescription: "Hridayam is a musical coming-of-age romantic drama that captured hearts across generations. It tracks the life of Arun Neelakandan from his college days to fatherhood. Kalyani plays Nithya, a pivotal character who brings stability and love into Arun's turbulent life.",
        director: "Vineeth Sreenivasan",
        cast: ["Pranav Mohanlal", "Darshana Rajendran", "Kalyani Priyadarshan"],
        images: [
            "https://wallpaperaccess.com/full/7988325.jpg"
        ]
    },
    {
        id: "maanaadu",
        title: "Maanaadu",
        role: "Seetha",
        year: "2021",
        genre: "Sci-Fi Thriller",
        trailer: "https://www.youtube.com/embed/3FuuZU21S80",
        status: "Released",
        description: "A man gets stuck in a time loop on the day of a political rally and must save the chief minister to break free.",
        fullDescription: "A blockbuster sci-fi action thriller involving a time loop. Maanaadu follows a man who gets stuck in a loop on the day of a political conference. He must figure out how to stop an assassination to break the cycle. Kalyani plays Seetha, who aids him in this high-stakes mission.",
        director: "Venkat Prabhu",
        cast: ["Silambarasan TR", "S. J. Suryah", "Kalyani Priyadarshan"],
        images: [
            "https://m.media-amazon.com/images/M/MV5BMTJkYjFhMWYtNGMxMy00YzQ5LThkNWYtY2U2NGNkZGNiNWE3XkEyXkFqcGc@._V1_FMjpg_UX1080_.jpg"
        ]
    }
];
